...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
                 ...
                 pool_size=7, linear_root=False, ouput_indice=[3,4,5,6], **args):
        super(DLA, self).__init__()
        self.channels = channels
        self.return_levels = return_levels
        self.num_classes = num_classes
        self.ouput_indice = ouput_indice
...
def main():
    ...
    input = torch.randn(1,3,1024,800).cuda()
    model = dla34(return_levels=True).cuda()
    for i in range(1000):        
        torch.cuda.synchronize()
        t = time.time()
        output = model(input)
        torch.cuda.synchronize()
        print('tttt ms ', (time.time()-t)*1000)
